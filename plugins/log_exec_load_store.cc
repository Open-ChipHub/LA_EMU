...
void print_data(char* dst, uint32_t size_shift, void* data) {
    switch (size_shift) {
        case 0: sprintf(dst, "%02x", *(uint8_t*)data);  break;
        case 1: sprintf(dst, "%04x", *(uint16_t*)data);  break;
        case 2: sprintf(dst, "%08x", *(uint32_t*)data);  break;
        case 3: sprintf(dst, "%016lx", *(uint64_t*)data);  break;
        case 4: sprintf(dst, "%016lx %016lx", *(uint64_t*)data, *((uint64_t*)data + 1));  break;
        case 5: sprintf(dst, "%016lx %016lx %016lx %016lx", *(uint64_t*)data, *((uint64_t*)data + 1), *((uint64_t*)data + 2), *((uint64_t*)data + 3));  break;
        default: assert(0);
    }
}
...
void emu_load(uint64_t vaddr, uint32_t size_shift, void* data) {
    char str_data[100];
    print_data(str_data, size_shift, data);
    fprintf(stderr, "load,vaddr,%016lx,data,%s\n", vaddr, str_data);
}
void emu_store(uint64_t vaddr, uint32_t size_shift, void* data) {
    char str_data[100];
    print_data(str_data, size_shift, data);
    fprintf(stderr, "store,vaddr,%016lx,data,%s\n", vaddr, str_data);
}
