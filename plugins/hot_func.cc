...
void my_emu_insn_before_curpc(void* env, uint64_t pc, uint32_t insn) {
    ++ pc_cnt[pc];
}
...
void my_emu_stop() {
    ...
    unordered_map<uint64_t, uint64_t> func_cnt;
    for (const auto& kv : pc_cnt) {
        int64_t index = sym_info_find_interval(elf_sym_info, kv.first);
        total_cnt += kv.second;
        if (index < 0) {
            func_cnt[-1] += kv.second;
        } else {
            func_cnt[index] += kv.second;
        }
    }
...
la_emu_plugin_ops my_op = {
    ...
    .emu_insn_before = my_emu_insn_before_curpc,
};
